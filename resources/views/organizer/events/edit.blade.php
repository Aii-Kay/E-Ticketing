...
<head>
    ...
    <meta name="description" content="Edit Event Studio - Organizer Panel">
...
<body class="antialiased text-slate-800 flex min-h-screen" x-data="{ sidebarOpen: false }">
    ...
    {{-- =======================================================================
         3. SIDEBAR NAVIGATION (ORGANIZER)
    ======================================================================= --}}
    ...
    <div class="lg:ml-72 flex-1 flex flex-col min-h-screen relative z-10 transition-all">
        ...
        <div class="p-6 lg:p-12 max-w-7xl mx-auto w-full">
            ...
            <div class="flex flex-col md:flex-row justify-between items-end gap-6 mb-12" data-aos="fade-down">
                <div>
                    <h1 class="text-4xl md:text-5xl font-display font-extrabold text-slate-900 mb-2">
                        Edit <span class="text-transparent bg-clip-text bg-gradient-to-r from-violet-600 to-indigo-600">Event</span>
                    </h1>
                    <p class="text-slate-500 text-lg">Perbarui informasi event Anda agar tetap menarik.</p>
                </div>
            ...
            {{-- Main Form Grid --}}
            {{-- x-data for Live Preview & Pre-filled Data --}}
            <form method="POST" action="{{ route('organizer.events.update', $event->id) }}"
                  enctype="multipart/form-data"
                  class="grid grid-cols-1 lg:grid-cols-3 gap-10"
                  x-data="{
                      imageUrl: '{{ $event->image ? asset($event->image) : "" }}',
                      eventName: '{{ addslashes($event->name) }}',
                      eventLoc: '{{ addslashes($event->location) }}',
                      eventDate: '{{ $event->date ? \Carbon\Carbon::parse($event->date)->format("Y-m-d") : "" }}',
                      eventTime: '{{ $event->time ? substr($event->time, 0, 5) : "" }}'
                  }">
                @csrf
                @method('PUT')
                ...
                <div class="lg:col-span-2 glass-form-panel p-8 md:p-12 space-y-8" data-aos="fade-up">
                    ...
                    {{-- 5. Visual --}}
                    <div>
                        <label>Update Banner Event</label>
                        <div class="input-group">
                            <input type="file"
                                   name="image"
                                   class="glass-input file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-violet-50 file:text-violet-700 hover:file:bg-violet-100"
                                   accept="image/*"
                                   @change="imageUrl = URL.createObjectURL($event.target.files[0])">
                            <i class="ph-duotone ph-upload-simple input-icon"></i>
                        </div>
                        <p class="text-[10px] text-slate-400 mt-2 ml-1 italic font-medium">*Biarkan kosong jika tidak ingin mengubah gambar.</p>
                    </div>
                            ...
                            <div class="preview-image-wrapper">
                                ...
                                <div class="absolute top-5 left-5 flex gap-2">
                                    <div class="bg-white/90 backdrop-blur-md rounded-xl px-3 py-1.5 text-xs font-bold text-violet-600 shadow-lg border border-white/50 flex items-center gap-1.5">
                                        <i class="ph-fill ph-eye"></i> Live Edit
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="p-8 space-y-6">
                                ...
                                <div class="p-4 bg-slate-50 rounded-2xl border border-slate-200 flex items-center justify-between">
                                    ...
                                    <div class="text-right">
                                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-100 text-emerald-700 text-[10px] font-bold border border-emerald-200">
                                            Existing
                                        </span>
                                    </div>
                                </div>
