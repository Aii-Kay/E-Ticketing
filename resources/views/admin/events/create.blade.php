...
<head>
    ...
    <meta name="description" content="Create Event Studio - Admin Panel">
    ...
    {{-- =======================================================================
         2. CUSTOM STYLES (The Genesis Studio Theme)
    ======================================================================= --}}
    <style>
        /* Base */
        ...
        .orb-1 { top: -10%; left: -10%; width: 60vw; height: 60vw; background: #a5b4fc; } /* Indigo */
        .orb-2 { bottom: -20%; right: -20%; width: 60vw; height: 60vw; background: #6ee7b7; animation-delay: -5s; } /* Emerald */
        .orb-3 { top: 40%; left: 30%; width: 40vw; height: 40vw; background: #fca5a5; opacity: 0.4; animation-delay: -10s; } /* Rose */
        ...
        .glass-input:focus + .input-icon { color: #4f46e5; transform: translateY(-50%) scale(1.1); }
    ...
    <div class="lg:ml-72 flex-1 flex flex-col min-h-screen relative z-10 transition-all">
        ...
        <div class="p-6 lg:p-12 max-w-7xl mx-auto w-full">
            ...
            <div class="flex flex-col md:flex-row justify-between items-end gap-6 mb-12" data-aos="fade-down">
                <div>
                    <h1 class="text-4xl md:text-5xl font-display font-extrabold text-slate-900 mb-2">
                        Genesis <span class="text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 to-teal-600">Studio</span>
                    </h1>
                ...
                <a href="{{ route('admin.events.index') }}" class="px-6 py-3 rounded-2xl bg-white border border-slate-200 text-slate-600 font-bold text-sm hover:bg-slate-50 transition-all shadow-sm flex items-center gap-2 group">
                    <i class="ph-bold ph-arrow-left group-hover:-translate-x-1 transition-transform"></i> Batal & Kembali
                </a>
            </div>
            ...
            @php
                ...
                // FIX: Paksa ambil data organizer dari DB jika controller tidak mengirimkannya
                // Ini memastikan card tidak kosong / error "Belum ada organizer"
                if (!isset($organizers)) {
                    $organizers = \App\Models\User::where('role', 'organizer')->get();
                }
            @endphp

            {{-- x-data for Live Preview --}}
...
{{-- 5. Visual --}}
                    ...
                    {{-- 6. ADMIN ONLY: Organizer Selector (FIXED) --}}
                    @if ($role === 'admin')
                        <div class="p-6 bg-slate-50/80 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden">
                            <div class="absolute top-0 right-0 w-20 h-20 bg-indigo-500/10 rounded-bl-full"></div>

                            <label class="!ml-0 !mb-3 flex items-center gap-2 text-indigo-600 font-extrabold">
                                <i class="ph-fill ph-crown text-lg"></i> Organizer Pemilik Event
                            </label>

                            @if ($organizers->isNotEmpty())
                                <div class="input-group">
                                    <select name="created_by" class="glass-input appearance-none cursor-pointer !bg-white" required>
                                        <option value="">-- Pilih Organizer --</option>
                                        @foreach ($organizers as $org)
                                            <option value="{{ $org->id }}" {{ old('created_by') == $org->id ? 'selected' : '' }}>
                                                {{ $org->name }} ({{ $org->email }}) - {{ ucfirst($org->status) }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <i class="ph-duotone ph-user-circle input-icon"></i>
                                    <i class="ph-bold ph-caret-down absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none"></i>
                                </div>
                            @else
                                <div class="flex items-start gap-3 text-amber-600 bg-amber-50 p-4 rounded-xl border border-amber-200">
                                    <i class="ph-fill ph-warning-circle text-2xl shrink-0 mt-0.5"></i>
                                    <div>
                                        <p class="text-sm font-bold">Belum ada user dengan role 'organizer'.</p>
                                        <p class="text-xs opacity-90 mt-1">Pastikan ada user yang mendaftar dan memiliki role 'organizer' di database.</p>
                                    </div>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="preview-card" data-tilt data-tilt-max="5" data-tilt-speed="400">
                            ...
                            <div class="preview-image-wrapper">
                                ...
                                <div class="absolute top-5 left-5 flex gap-2">
                                    <div class="bg-white/90 backdrop-blur-md rounded-xl px-3 py-1.5 text-xs font-bold text-emerald-600 shadow-lg border border-white/50 flex items-center gap-1.5">
                            ...
                            <div class="p-8 space-y-6">
                                <div>
                                    <h3 class="text-2xl font-bold text-slate-900 leading-tight mb-2 line-clamp-2"
                                        x-text="eventName ? eventName : 'Nama Event Anda'"></h3>
